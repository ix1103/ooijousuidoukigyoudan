...
export default function WaterQualityPage() {
    ...
    const managementResultsFallback = [
    ...
    const managementResults = loading ? [] : (documents.filter(d => d.category.includes('水質管理目標')).length > 0 ? documents.filter(d => d.category.includes('水質管理目標')) : managementResultsFallback);
                        ...
                        <div className="space-y-8">
                            ...
                            <div>
                                ...
                                <div className="space-y-3">
                                    {loading ? (
                                        <div className="h-16 bg-slate-100/50 animate-pulse rounded-2xl"></div>
                                    ) : managementResults.map((doc: any, idx) => (
                                        <a
                                            key={doc.id || idx}
                        ...
                        <div>
                            ...
                            <div className="bg-white p-6 md:p-8 rounded-[2rem] border border-slate-200 shadow-sm relative overflow-hidden">
                                ...
                                <div className="relative z-10 space-y-3">
                                    {loading ? (
                                        <>
                                            <div className="h-16 bg-slate-100/50 animate-pulse rounded-xl"></div>
                                            <div className="h-16 bg-slate-100/50 animate-pulse rounded-xl"></div>
                                        </>
                                        ...
                                        <a
                                            ...
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            className="group flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-slate-50/50 rounded-xl border border-transparent hover:border-slate-200 hover:bg-white hover:shadow-sm transition-all duration-300 gap-3"
                                        >
                                            <div className="flex items-center gap-3 w-full sm:w-auto overflow-hidden">
                                                <div className="w-8 h-8 shrink-0 rounded-full bg-secondary-vibrant/10 flex items-center justify-center text-secondary-vibrant group-hover:bg-secondary-vibrant group-hover:text-primary-deep transition-colors">
                                                    <Download size={16} />
                                                </div>
                                                <span className="font-bold text-primary-deep group-hover:text-primary-main transition-colors text-sm md:text-base truncate">
                                            ...
                                            <ChevronRight className="hidden sm:block text-slate-300 group-hover:text-primary-main group-hover:translate-x-1 transition-all shrink-0" size={18} />
                                        </a>
                                    ))}
