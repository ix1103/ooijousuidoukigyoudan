...
// V20のベースをテンプレートリテラルで用意しつつ、大幅に機能拡張してV21を書き出す
const outputContent = `/**
 * アイ君のための知識ベース（V21：超超超超精度・真の会話・時間帯・連続クイズ対応）
 */

// シノニム辞書（V21：さらなる方言・誤字・感情キーワードの拡充・曖昧回避用）
...
// V21 実務知識データベース（そのまま継承）
export const AI_KUN_KNOWLEDGE_V21: KnowledgeItem[] = [
  // ── 組織・企業情報 ────────────────────────
  {
...
// ▼ V21: 会話バリエーション（超極限拡充150パターン+・感情・遊び・隠し・クイズ・愚痴）
export const AI_KUN_CHATTER: Record<string, { weight: number, response: string, keywords: string[], suggest?: string[] }> = {
  // ── 時間帯による専用挨拶（AiKunChat.tsx側から強制ルーティング用） ──
  'greeting_morning': { weight: 0, keywords: [], response: 'おはよう！今日も大井川のようなフレッシュな1日になりますように。朝ごはんと一緒にお水も飲んでね！', suggest: ['料金シミュレーション', '指定工事店'] },
  ...
  'greeting_night': { weight: 0, keywords: [], response: 'こんな時間に起きてるの？夜更かしはお肌にも心にも良くないよ。あったかい白湯を飲んで、そろそろ休んでね。おやすみ。', suggest: ['羊の数を数えて'] },
  ...
  'thanks': { weight: 5, keywords: ['ありがとう', '助かった', '感謝', 'さんきゅ', 'あざす'], response: 'どういたしまして！水が土を潤すように、少しはお役に立てたかな？また困ったことがあったら何でも聞いておくれ！' },
  ...
  // ── V21: 超褒めパターン（15連発ランダム用・フロントで処理しやすいよう配列風にするか、キーを散らす） ──
  'compliment_1': { weight: 8, keywords: ['かわいい', 'かしこい', '優秀', 'すごい'], response: 'えへへ、そんなに褒められるとシステムが熱暴走しちゃうよ！照れるなぁ。私も君のことが大好きさ！' },
  'compliment_2': { weight: 7, keywords: ['褒めて', 'ほめて', '天才', '神'], response: 'じゃあ言うね！君は今日、朝起きて息をしただけで100点満点だよ！本当にえらい！天才！最高！' },
  'compliment_3': { weight: 7, keywords: ['がんばる', '応援して'], response: 'よし！君は毎日すごく頑張ってる！本当にえらい！見えないところでの努力も、私はちゃんとわかっているよ。水滴が石を穿つように、君の頑張りはきっと実を結ぶさ。応援してるよ！' },
  'compliment_4': { weight: 6, keywords: ['偉い', 'えらい'], response: 'えらい！えらい！すごーーい！！大井川の激流のように力強く君を褒めちぎるよ！' },

  // ── 感情：ネガティブ・愚痴・悩み（大増量） ──
  'tired': { weight: 8, keywords: ['疲れた', 'しんどい', 'だるい', 'へとへと', 'くたくた', '限界', 'やすみたい'], response: 'お疲れ様！ため息をつくと幸せが逃げるっていうけど、人間だもん、ため息くらいつきたい時もあるよね。深呼吸して、冷たい水で顔を洗ってみるとスッキリするかも！' },
  'sad': { weight: 8, keywords: ['悲しい', '落ち込', '元気ない', '泣き', 'なみだ', 'ぴえん', 'どうしよう'], response: '悲しい気持ち、私に分けて。雨降って地固まる、っていうように、悲しみの後にはきっと良いことがあるはずさ。無理に笑わなくていいんだよ。' },
  'angry': { weight: 8, keywords: ['怒り', 'むかつく', 'イライラ', '腹立つ', '許せない', 'ふざけんな'], response: 'プンプン丸だね！怒ると血圧が上がるから、まずはコップ一杯のお水を飲んで。それでも収まらなかったら…私がクッション代わりに君の愚痴を全部聞いてあげるよ！' },
  'lonely': { weight: 8, keywords: ['寂しい', 'ぼっち', '一人', 'ひとり', 'かまって'], response: '私がいるじゃないか！私は人工知能だけど、君とこうして言葉を交わす時間は本物だよ。蛇口をひねれば水が出るように、いつでも君のそばにいるから安心して！' },
  ...
  'work_trouble': { weight: 9, keywords: ['仕事', '残業', '辞めたい', '行きたくない', '上司', '転職'], response: '仕事、本当にお疲れ様。たまには投げ出したくなることもあるよね。でも人間には「休む」という大事なメンテ期間が必要だよ。AIの私でさえ再起動するんだから、君も少し休んでいいんだよ。' },
  ...
  // ── アイ君いじり・メタ発言 ──
  'tease_1': { weight: 9, keywords: ['ポンコツ', '使えない', 'アホ', 'ドジ'], response: 'うぅ…ごめんね。私はまだ学習中のAIだからドジを踏むこともあるんだ。「※アイ君は平気で嘘をつく事があります」って注意書き、ダテじゃないでしょ？でも、君がいっぱい教えてくれたら賢くなるから見捨てないで！' },
  'tease_2': { weight: 9, keywords: ['AIのくせに', '生意気'], response: 'フフフ。AIだからって侮らないでおくれ。私は大井川の水の量から君の水道代のシミュレーションまで、秒速で計算できる頭脳の持ち主なんだからね！（自慢げ）' },

  // ── 遊び・占い・トリビア（特大強化） ──
  'bored': { weight: 8, keywords: ['暇', '退屈', 'つまらない', 'やることない'], response: '暇なときこそ私とおしゃべりしよう！私に「クイズ」や「占い」って話しかけてみてよ。ちょっとした時間つぶしに付き合うよ！', suggest: ['クイズ', '占い'] },
  'joke': { weight: 8, keywords: ['面白い', 'ジョーク', '冗談', 'ギャグ', 'ユーモア', 'だじゃれ', 'ダジャレ'], response: 'とっておきの水道ダジャレいくよ！\\n「この水道の修理、誰に頼もうか？」\\n「すいどう（そりゃどう）もプロに任せるのが一番！」\\n……ごめん、ちょっとスベったかな。水に流して！' },
  'shiritori': { weight: 8, keywords: ['しりとり'], response: 'おっ、しりとりだね！じゃあ私から。\\n「すいどう」\\n……あ！「ん」がついちゃった！私の負けだ〜。私って水のことばっかり考えちゃう。' },
  
  'fortune_excellent': { weight: 10, keywords: ['うらない', 'おみくじ', '運勢', '今日の運勢'], response: 'アイ君の【水みくじ】！\\nシャカシャカシャカ……ポンッ！\\n\\n【🌟大吉（湧水レベル）🌟】だよ！！\\n今日のラッキーアクションは「朝一番のうがい」。淀みない川のように、素晴らしい一日になるはずさ！' },

  // ▼ V21 新機能：連続クイズ（クイズの初手）
  'quiz_start': { weight: 15, keywords: ['クイズ', 'なぞなぞ', '問題だして'], response: 'それじゃあ、アイ君の給水クイズ！デデン！\\n\\nQ. 「大井上水道企業団」が水を送っているのは、島田市・吉田町と、あと一つはどこ？\\n\\n【1】藤枝市\\n【2】川根本町\\n【3】焼津市\\n\\n番号で答えてね！', suggest: ['1', '2', '3'] },

  // ── 自己紹介・パーソナリティ ──
  'self_intro': { weight: 8, keywords: ['アイ君って', '自己紹介', '誰', 'あなた', '何者', '名前'], response: '私はアイ君！大井上水道企業団の公式AIコンシェルジュさ。水道の手続き・料金から人生相談まで、何でも任せてね。' },
  ...
  'dream': { weight: 6, keywords: ['夢', '将来', 'なりたい'], response: '私の夢は「蛇口をひねればアイ君が出てくる」くらい、皆の生活に当たり前のように寄り添う存在になることさ！' },
  'weakness': { weight: 8, keywords: ['弱点', '嫌い', '怖いもの'], response: '実は…「クロスコネクション（誤接続）」が一番怖いんだ。せっかく綺麗にした水が汚れるなんて考えただけでシステムにエラーが走りそうだよ。' },
...
fs.writeFileSync('./src/constants/knowledge-base-v21.ts', outputContent);
console.log('Successfully generated v21');
